...
/*   Updated: 2020/08/25 20:04:58 by deddara          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "engine.h"

static int make_header(char *bmp_fheader, char *bmpinfheader, t_raycast *ray, t_data *img)
{
	int file_size;
...
static int write_in_header(char *bmp_fheader, char *bmpinfheader, t_raycast *ray, t_data *img)
{
	int fd;
	char *fname;

	fname = "screenshot.bmp";
	if ((fd = open(fname, O_RDWR | O_CREAT, 664)) < 0)
		return (0);
