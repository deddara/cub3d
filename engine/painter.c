...
/*   Updated: 2020/08/18 17:11:26 by deddara          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	texture_clalcs(t_raycast *ray, t_paint *paint, t_map *map)
{
	...
	paint->tex_x = (int)(paint->wall_x * (double)(ray->txtr_so.width));
	if (ray->wall_side == 2 || ray->wall_side == 1)
		paint->tex_x = ray->txtr_so.width - paint->tex_x - 1;
	paint->step = 1.0 * ray->txtr_so.height / ray->wall_height;
	paint->tex_pos = \
...
void		paint_map(t_raycast *ray, t_map *map, t_data *img, int x)
{
	...
	{
		paint.tex_y = (int)paint.tex_pos & (ray->txtr_so.height - 1);
		if (ray->wall_side == 0)
