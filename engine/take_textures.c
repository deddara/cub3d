...
/*   Updated: 2020/08/28 16:36:56 by deddara          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(ray->txtr_so.img = mlx_xpm_file_to_image(ray->vars->mlx, ray->map->so,\
		&ray->txtr_so.width, &ray->txtr_so.height)))
		return (0);
	if (!(ray->txtr_no.img = mlx_xpm_file_to_image(ray->vars->mlx, ray->map->no,\
	&ray->txtr_no.width, &ray->txtr_no.height)))
		return (0);
	if (!(ray->txtr_ea.img = mlx_xpm_file_to_image(ray->vars->mlx, ray->map->ea,\
		&ray->txtr_ea.width, &ray->txtr_ea.height)))
		return (0);
	if (!(ray->txtr_we.img = mlx_xpm_file_to_image(ray->vars->mlx, ray->map->we,\
		&ray->txtr_we.width, &ray->txtr_we.height)))
		return (0);
	if (!(ray->txtr_s.img = mlx_xpm_file_to_image(ray->vars->mlx, ray->map->s,\
